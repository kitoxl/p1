              <div>
                ...
                <select value={attackMethod} onChange={(e) => setAttackMethod(e.target.value)} className="w-full px-2 py-2 border rounded">
              ...
              <div>
                ...
                <input type="number" value={packetSize} onChange={(e) => setPacketSize(Number(e.target.value))} className="w-full px-2 py-2 border rounded" min={1} max={1500} />
              </div>
              <div>
                <label className="block text-sm">Duration (s)</label>
                <input type="number" value={duration} onChange={(e) => setDuration(Number(e.target.value))} className="w-full px-2 py-2 border rounded" min={1} max={3600} />
              </div>
              <div>
                <label className="block text-sm">Packet Delay (ms)</label>
                <input type="number" value={packetDelay} onChange={(e) => setPacketDelay(Number(e.target.value))} className="w-full px-2 py-2 border rounded" min={1} max={10000} />
              </div>
            </div>
          </section>
              ...
              export default function App(): JSX.Element {
                ...
                useEffect(() => {
                  socket.on("stats", (data: any) => {
                    setStats((old) => ({ pps: data.pps ?? old.pps, bots: data.bots ?? old.bots, totalPackets: data.totalPackets ?? old.totalPackets }));
                ...
                const addLog = (message: string) => setLogs((p) => [message, ...p].slice(0, 100));
                ...
                const startAttack = (isQuick?: boolean) => {
                  ...
                  socket.emit("startAttack", { target, packetSize, duration, packetDelay, attackMethod, simulation: simulationMode });
                };
                ...
                function saveCurrentPreset() {
                  try {
                    ...
                    if (!name) return;
                    const newPreset = { name, attackMethod, packetSize, duration, packetDelay, target };
                    const next = [newPreset, ...presets.filter((p) => p.name !== name)].slice(0, 20);
                    setPresets(next);
                    try {
                ...
                return (
                  <div className={`w-screen min-h-screen p-8 bg-gradient-to-br ${darkMode ? "from-gray-900 to-gray-800" : "from-pink-100 to-blue-100"}`}>
                    <audio ref={audioRef} src="/audio.mp3" />
                    <div className="max-w-4xl mx-auto space-y-6">
                      <header className="flex items-center justify-between">
                        <div>
                          <h1 className="text-3xl font-bold text-pink-500">Miku Miku Beam</h1>
                          <p className="text-sm text-gray-500">Real-time visualizer & controller (demo)</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button onClick={() => setShowConfig(true)} className="px-3 py-2 rounded bg-slate-800 text-white flex items-center gap-2"><ScrollText className="w-4 h-4"/> Config</button>
                          <button onClick={() => { fetchHistory(); }} className="px-3 py-2 rounded border">History</button>
                          <button onClick={() => setDarkMode((d) => !d)} className="px-3 py-2 rounded border">{darkMode ? 'Light' : 'Dark'}</button>
                        </div>
                      </header>
                      ...
                      <section className="p-4 bg-white dark:bg-gray-900 rounded shadow">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <input value={target} onChange={(e) => setTarget(e.target.value)} className="w-full px-3 py-2 border rounded" placeholder="example.com or http://..." />
                          </div>
                          <div className="flex items-end gap-2">
                            <button onClick={() => isAttacking ? stopAttack() : startAttack()} className={`px-4 py-2 rounded text-white ${isAttacking ? 'bg-red-500' : 'bg-pink-500'}`}>{isAttacking ? 'Stop' : 'Start'}</button>
                            <button onClick={() => startAttack(true)} className="px-3 py-2 rounded bg-cyan-500 text-white">Quick</button>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-4">
                          ...
                          <button onClick={saveCurrentPreset} className="ml-auto px-3 py-1 rounded border">Save preset</button>
                        </div>
                      </section>

                      <section className="grid grid-cols-3 gap-4">
                        <div className="p-4 rounded bg-white dark:bg-gray-900 shadow">
                          <div className="text-sm text-pink-600 flex items-center gap-2"><Zap className="w-4 h-4"/>Packets/sec</div>
                          <div className="text-2xl font-bold">{stats.pps.toLocaleString()}</div>
                        </div>
                        <div className="p-4 rounded bg-white dark:bg-gray-900 shadow">
                          <div className="text-sm text-pink-600 flex items-center gap-2"><Bot className="w-4 h-4"/>Active Bots</div>
                          <div className="text-2xl font-bold">{stats.bots.toLocaleString()}</div>
                        </div>
                        <div className="p-4 rounded bg-white dark:bg-gray-900 shadow">
                          <div className="text-sm text-pink-600 flex items-center gap-2"><Wifi className="w-4 h-4"/>Total Packets</div>
                      ...
                      <section className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-white dark:bg-gray-900 rounded shadow">
                          ...
                          {clientSpecs ? (
                            <div className="text-sm space-y-1">
                              <div><b>User agent:</b> {clientSpecs.userAgent}</div>
                              <div><b>Platform:</b> {clientSpecs.platform}</div>
                              <div><b>Language:</b> {clientSpecs.language}</div>
                              <div><b>CPU cores:</b> {clientSpecs.cores ?? 'unknown'}</div>
                              <div><b>Device memory (GB):</b> {clientSpecs.deviceMemory ?? 'unknown'}</div>
                              <div><b>Screen:</b> {clientSpecs.screen}</div>
                              <div><b>GPU:</b> {clientSpecs.gpu || 'unknown'}</div>
                            </div>
                          ) : (
                        ...
                        <div className="p-4 bg-white dark:bg-gray-900 rounded shadow">
                          ...
                          {serverSpecs ? (
                            <div className="text-sm space-y-1">
                              <div><b>Platform:</b> {serverSpecs.platform}</div>
                              <div><b>Arch:</b> {serverSpecs.arch}</div>
                              <div><b>Node:</b> {serverSpecs.nodeVersion}</div>
                              <div><b>CPUs:</b> {serverSpecs.cpus}</div>
                              <div><b>Total RAM (MB):</b> {Math.round(serverSpecs.totalMem / 1024 / 1024)}</div>
                              <div><b>Free RAM (MB):</b> {Math.round(serverSpecs.freeMem / 1024 / 1024)}</div>
                              <div><b>Uptime (s):</b> {Math.round(serverSpecs.uptime)}</div>
                            </div>
                          ) : (
                      ...
                      <section className="p-4 bg-white dark:bg-gray-900 rounded shadow font-mono text-sm">
                        ...
                        <div className="max-h-48 overflow-auto">
                          {logs.length === 0 ? <div className="italic text-gray-500">No logs yet</div> : logs.map((l, i) => <div key={i} className="py-1">{l}</div>)}
                        </div>
                      </section>

                      {showConfig && <ConfigureProxiesAndAgentsView onClose={() => setShowConfig(false)} />}
                        ...
                        <div className="fixed inset-0 z-50 flex items-start justify-center p-8 bg-black/50">
                          <div className="w-[90%] max-h-[80%] overflow-auto bg-white dark:bg-gray-900 text-black dark:text-white rounded-md p-4">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <button onClick={() => setShowHistory(false)} className="px-3 py-1 rounded bg-gray-200 dark:bg-gray-800">Close</button>
                              </div>
                            </div>
                            {history.length === 0 ? (
                              ...
                              history.slice().reverse().map((h: any, idx: number) => (
                                <div key={idx} className="p-2 border-b last:border-b-0">
                                  <div className="text-sm">{new Date(h.timestamp).toLocaleString()} — <b>{h.attackMethod}</b> {h.target}</div>
                                  <div className="text-xs text-gray-500 dark:text-gray-400">{h.status} {h.error ? ` - ${h.error}` : ''}</div>
                                </div>
                              ))
                            )}
                          </div>
