...
const attackWorkers: { [key in AttackMethod]: string } = {
  ...
  https_flood: "./workers/httpsFloodAttack.js",
...
// History helpers
const HISTORY_FILE = join(currentPath(), "data/history.json");
...
function appendHistory(entry: any) {
  try {
    ...
    arr.push(entry);
    writeFileSync(HISTORY_FILE, JSON.stringify(arr, null, 2), "utf8");
  } catch (e) {
...
io.on("connection", (socket) => {
  ...
  socket.on("startAttack", (params) => {
    ...
    worker.on("error", (error) => {
      ...
      appendHistory({ target, attackMethod, status: "error", error: error.message, timestamp: Date.now() });
    });

    worker.on("exit", (code) => {
      console.log(`Worker exited with code ${code}`);
      socket.emit("attackEnd");
      appendHistory({ target, attackMethod, status: "finished", code, timestamp: Date.now() });
    });
